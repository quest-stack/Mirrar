...
/**
 * Vibe Coding スタイル向けのAI連携プラグイン
 *
 * Vibe Coding = Claude Code、Cursor、Windsurf、GitHub Copilot等の
 * AIコーディングツールを使った開発スタイル
 *
 * このプラグインは、クリップボード経由でプロンプトを共有するため、
 * APIキー不要・完全無料で使用できます。
 */
export class AICodingAssistPlugin extends AIPlugin {
  constructor() {
    super('AI Coding Assistant', null); // No endpoint needed for clipboard approach
  }
    ...
    try {
      ...
      return {
        ...
        action: {
          type: 'guide',
          steps: [
            'Claude Code、Cursor、Windsurf等のAIツールを開く',
...
// 後方互換性のためのエイリアス（既存コードで VibeCoderPlugin を使っている場合）
export const VibeCoderPlugin = AICodingAssistPlugin;
